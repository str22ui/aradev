...
<div class="max-w-6xl mx-auto">
  ...
  <div class="bg-gradient-to-r from-blue-600 to-blue-800 text-white p-8 rounded-lg shadow-lg mb-6">
    <h2 class="text-3xl font-bold mb-2">Selamat Datang, {{ $user->name ?? 'Agent' }}!</h2>
    ...
    <div class="lg:col-span-1 bg-white rounded-lg shadow-md p-6">
      ...
      <div class="space-y-3">
        ...
        <div class="border-b pb-2">
          ...
          <p class="text-gray-800">{{ $user->joined_at ? \Carbon\Carbon::parse($user->joined_at)->format('d M Y') : '-' }}</p>
        </div>
      </div>
    </div>
      ...
      <div class="bg-white rounded-lg shadow-md p-6 border-l-4 border-purple-500">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-3xl font-bold text-gray-800 mt-2">{{ $user->commission_rate ?? '0' }}%</p>
          </div>
      ...
      <div class="bg-white rounded-lg shadow-md p-6 border-l-4 border-green-500">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-3xl font-bold text-gray-800 mt-2">Rp {{ number_format($user->total_sales ?? 0, 0, ',', '.') }}</p>
          </div>
      ...
      <div class="bg-white rounded-lg shadow-md p-6 border-l-4 border-yellow-500 md:col-span-2">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-4xl font-bold text-gray-800 mt-2">Rp {{ number_format($user->total_commission ?? 0, 0, ',', '.') }}</p>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
      ...
      <div class="bg-gray-50 p-4 rounded-lg">
        <p class="text-gray-700 leading-relaxed">{{ $user->address ?? 'Alamat belum diisi' }}</p>
      </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
      ...
      <div class="space-y-3">
        <div class="flex justify-between items-center py-2 border-b">
          ...
          <span class="text-gray-800 font-semibold">{{ $user->perumahan->nama ?? 'Tidak terdaftar' }}</span>
        </div>
        <div class="flex justify-between items-center py-2 border-b">
          <span class="text-gray-600 font-medium">Lokasi:</span>
          <span class="text-gray-800 font-semibold">{{ $user->perumahan->lokasi ?? '-' }}</span>
        </div>
        <div class="flex justify-between items-center py-2">
          <span class="text-gray-600 font-medium">ID Perumahan:</span>
          <span class="text-gray-800 font-semibold">{{ $user->perumahan_id ?? '-' }}</span>
        </div>
      </div>
    </div>
  </div>
